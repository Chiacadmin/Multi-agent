...
async function openAlexSearch(q, start, end, limit, timeoutMs = 6000) {
    ...
    if (!resp.ok)
        throw new Error(`OpenAlex ${resp.status}`);
...
async function arxivSearch(q, start, end, limit, timeoutMs = 4000) {
    ...
    const items = entries.map((e) => {
        ...
        const absUrl = links.find((l) => l["@_title"] === "abstract" || l["@_rel"] === "alternate")?.["@_href"];
        const pdfUrl = links.find((l) => (l["@_type"] || "").includes("pdf"))?.["@_href"];
        const year = e.published ? Number(String(e.published).slice(0, 4)) : undefined;
        return {
            title: (e.title ?? "").toString(),
            authors: (Array.isArray(e.author) ? e.author : [e.author]).map((a) => a?.name).filter(Boolean),
...
const TOOLS = {
    async search_papers(args) {
        const { query, start, end, limit = 10, includeArxiv = false } = args;
        if (!query || !start || !end)
            throw new Error("Missing required fields: query, start, end");
        const tasks = [
    ...
    async get_pdf_url(args) {
        const { id } = args;
        if (!id)
            throw new Error("Missing id");
        if (/^10\./i.test(id))
            return { pdfUrl: `https://doi.org/${id}` };
        ...
        if (/^\d{4}\.\d{4,5}(v\d+)?$/i.test(id))
            return { pdfUrl: `https://arxiv.org/pdf/${id}.pdf` };
        return { pdfUrl: id };
    }
};
// ----- JSON-RPC stdio loop -----
...
rl.on("line", async (line) => {
    ...
    try {
        ...
        if (raw.method === "tools/call") {
            ...
            const fn = TOOLS[name];
            if (!fn) {
                send(err(id, -32601, `Unknown tool: ${name}`));
                return;
            }
